...
namespace Tour_ENDI_TourStub1
{
    public class SatelliteOrbitFall : MonoBehaviour
    {
        ...
        [Header("Настройки системы частиц")]
        [SerializeField] private ParticleSystem _particleSystem; // Система частиц для эффекта разрушения
        ...
        {
            if (other.gameObject.name == "ParticleTrigger")
            {
                _particleSystem.Play();
                StartCoroutine(RequestNextStageWithDelay());
            }
        }
        ...
        private IEnumerator RequestNextStageWithDelay()
        {
